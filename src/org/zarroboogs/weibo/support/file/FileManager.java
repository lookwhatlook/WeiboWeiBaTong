...
public class FileManager {
	 ...
	 * dir/Android/data/org.zarroboogs.weibo) with Root Explorer, uninstall
			...
			if (path != null) {
				...
				if (!cantReadBecauseOfAndroidBugPermissionProblem) {
					...
					activity.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							new AlertDialog.Builder(activity).setTitle(R.string.something_error).setMessage(R.string.please_deleted_cache_dir)
	...
	public File getAlbumStorageDir(String albumName) {
		...
		if (!file.mkdirs()) {
			AppLoggerUtils.e("Directory not created");
		}
		return file;
	}
	...
	public static String getKKConvertPicTempFile() {
		...
		if (!isExternalStorageMounted()) {
			...
			return getSdCardPath() + File.separator + "kk_convert" + System.currentTimeMillis() + ".jpg";
		}
	}
	...
	public static File createNewFileInSDCard(String absolutePath) {
		if (!isExternalStorageMounted()) {
			AppLoggerUtils.e("sdcard unavailiable");
			return null;
		}
	...
	public static boolean saveToPicDir(String path) {
		...
		String newPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).getAbsolutePath() + File.separator + WEICIYUAN
				+ File.separator + name;
		try {
