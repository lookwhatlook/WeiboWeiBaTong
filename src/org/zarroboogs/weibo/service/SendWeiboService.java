...
public class SendWeiboService extends Service {
	...
	private class WeiboSendTask extends MyAsyncTask<Void, Long, Void> {
		...
		public WeiboSendTask(int lastNotificationId, String token, AccountBean account, String picPath, String content, GeoBean geoBean,
				StatusDraftBean statusDraftBean) {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			Notification.Builder builder = new Notification.Builder(SendWeiboService.this).setTicker(getString(R.string.sending))
					.setContentTitle(getString(R.string.sending)).setContentText(content).setOnlyAlertOnce(true).setOngoing(true)
			...
			if (Utility.isJB()) {
				...
				IntentFilter intentFilter = new IntentFilter("org.zarroboogs.weibo.SendWeiboService.stop." + String.valueOf(notificationId));

				registerReceiver(receiver, intentFilter);

				Intent broadcastIntent = new Intent("org.zarroboogs.weibo.SendWeiboService.stop." + String.valueOf(notificationId));
		...
		protected void onProgressUpdate(Long... values) {
			...
			if (values.length > 0) {
				...
				if (data != -1) {
					...
					Notification.Builder builder = new Notification.Builder(SendWeiboService.this).setTicker(getString(R.string.send_photo))
							.setContentTitle(getString(R.string.background_sending)).setNumber((int) (r * 100)).setContentText(content)
							.setProgress((int) size, (int) data, false).setOnlyAlertOnce(true).setOngoing(true).setSmallIcon(R.drawable.upload_white);
					...
					Notification.Builder builder = new Notification.Builder(SendWeiboService.this).setTicker(getString(R.string.send_photo))
							.setContentTitle(getString(R.string.wait_server_response)).setContentText(content).setNumber(100).setProgress(100, 100, false)
		...
		private void showFailedNotification(final WeiboSendTask task) {
			Notification.Builder builder = new Notification.Builder(SendWeiboService.this).setTicker(getString(R.string.send_failed))
			...
			Intent notifyIntent = WriteWeiboActivity.startBecauseSendFailed(SendWeiboService.this, account, content, picPath, geoBean, statusDraftBean,
		...
		private void showSuccessfulNotification(final WeiboSendTask task) {
			Notification.Builder builder = new Notification.Builder(SendWeiboService.this).setTicker(getString(R.string.send_successfully))
