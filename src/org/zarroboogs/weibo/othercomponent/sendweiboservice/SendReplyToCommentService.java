...
public class SendReplyToCommentService extends Service {
	...
	private class WeiboSendTask extends MyAsyncTask<Void, Long, Void> {
		...
		public WeiboSendTask(AccountBean account, String token, String content, String repostContent, CommentBean oriMsg, ReplyDraftBean replyDraftBean) {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			Notification.Builder builder = new Notification.Builder(SendReplyToCommentService.this).setTicker(getString(R.string.sending_comment))
		...
		private void showSuccessfulNotification(final WeiboSendTask task) {
			Notification.Builder builder = new Notification.Builder(SendReplyToCommentService.this).setTicker(getString(R.string.send_successfully))
			...
			LocalBroadcastManager.getInstance(SendReplyToCommentService.this).sendBroadcast(
					new Intent(AppEventAction.buildSendCommentOrReplySuccessfullyAction(oriMsg.getStatus())));
		}

		private void showFailedNotification(final WeiboSendTask task) {
			Notification.Builder builder = new Notification.Builder(SendReplyToCommentService.this).setTicker(getString(R.string.send_failed))
			...
			Intent notifyIntent = WriteReplyToCommentActivity.startBecauseSendFailed(SendReplyToCommentService.this, account, content, oriMsg, replyDraftBean,
