...
public class BigTextNotificationService extends NotificationServiceHelper {
	...
	public static Intent newIntent(AccountBean accountBean, MessageListBean mentionsWeiboData, CommentListBean commentsToMeData,
			CommentListBean mentionsCommentData, UnreadBean unreadBean, Intent clickNotificationToOpenAppPendingIntentInner, String ticker, int currentIndex) {
	...
	private void buildNotification(String uid) {
		...
		Notification.Builder builder = new Notification.Builder(getBaseContext()).setTicker(ticker).setContentText(ticker)
				...
				.setContentIntent(getPendingIntent(clickToOpenAppPendingIntentInner, itemBean, accountBean)).setOnlyAlertOnce(true);
						...
						try {
							...
							if (Utility.isDebugMode()) {
								...
								new Handler(Looper.getMainLooper()).post(new Runnable() {
									@Override
									public void run() {
										Toast.makeText(getApplicationContext(), "weiciyuan:remove notification items" + System.currentTimeMillis(),
		...
		Utility.registerReceiverIgnoredReceiverHasRegisteredHereException(GlobalContext.getInstance(), valueWrapper.clearNotificationEventReceiver,
				intentFilter);
		...
		if (count > 1) {
			...
			Intent nextIntent = BigTextNotificationService.newIntent(accountBean, mentionsWeibo, commentsToMe, mentionsComment, unreadBean,
					clickToOpenAppPendingIntentInner, ticker, nextIndex);
			PendingIntent retrySendIntent = PendingIntent.getService(BigTextNotificationService.this, accountBean.getUid().hashCode(), nextIntent,
					PendingIntent.FLAG_UPDATE_CURRENT);
	...
	private PendingIntent getPendingIntent(Intent clickToOpenAppPendingIntentInner, Parcelable itemBean, AccountBean accountBean) {
		...
		PendingIntent pendingIntent = PendingIntent.getActivity(getBaseContext(), getMentionsWeiboNotificationId(accountBean),
