...
public class AtUsersDBTask {
	...
	public static List<AtUserBean> get(SQLiteDatabase db, String accountId) {
		...
		String sql = "select * from " + AtUsersTable.TABLE_NAME + " where " + AtUsersTable.ACCOUNTID + "  = " + accountId + " order by " + AtUsersTable.ID
				+ " desc";
	...
	private static void reduce(SQLiteDatabase db, String accountId) {
		...
		if (needDeletedNumber > 0) {
			String sql = " delete from " + AtUsersTable.TABLE_NAME + " where " + AtUsersTable.ID + " in " + "( select " + AtUsersTable.ID + " from "
					+ AtUsersTable.TABLE_NAME + " where " + AtUsersTable.ACCOUNTID + " in " + "(" + accountId + ") order by " + AtUsersTable.ID + " asc limit "
					+ needDeletedNumber + " ) ";
