...
public class NotificationDBTask {
	...
	public static Set<String> getUnreadMsgIds(String accountId, UnreadDBType type) {
		...
		String sql = "select * from " + NotificationTable.TABLE_NAME + " where " + NotificationTable.ACCOUNTID + "  = " + accountId + " and "
				+ NotificationTable.TYPE + " = " + type.getValue() + " order by " + NotificationTable.ID + " asc";
	...
	private static void cleanUnread(String accountId, UnreadDBType type) {
		String sql = "delete from " + NotificationTable.TABLE_NAME + " where " + NotificationTable.ACCOUNTID + " in " + "(" + accountId + ")" + " and "
				+ NotificationTable.TYPE + " = " + type.getValue();

		getWsd().execSQL(sql);
	}

	private static boolean needCleanDB(String accountId) {
		String searchCount = "select count(" + NotificationTable.MSGID + ") as total" + " from " + NotificationTable.TABLE_NAME + " where "
