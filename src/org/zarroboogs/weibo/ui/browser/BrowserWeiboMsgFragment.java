...
import org.zarroboogs.weibo.ui.adapter.BrowserWeiboMsgCommentAndRepostAdapter;
...
import org.zarroboogs.weibo.ui.loader.CommentsByIdMsgLoader;
import org.zarroboogs.weibo.ui.loader.RepostByIdMsgLoader;
...
public class BrowserWeiboMsgFragment extends AbstractAppFragment implements IRemoveItem {
	...
	private View.OnClickListener repostContentOnClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (isNotLink && !isDeleted) {
				startActivity(BrowserWeiboMsgActivity.newIntent(GlobalContext.getInstance().getAccountBean(), msg.getRetweeted_status(), GlobalContext
	...
	private AdapterView.OnItemLongClickListener repostOnItemLongClickListener = new AdapterView.OnItemLongClickListener() {
		...
		public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if (position - getListView().getHeaderViewsCount() < repostList.getSize() && position - getListView().getHeaderViewsCount() >= 0
			...
			if (position - listView.getHeaderViewsCount() < commentList.getSize() && position - listView.getHeaderViewsCount() >= 0) {
				if (actionMode != null) {
					...
					actionMode = getActivity().startActionMode(
							new CommentSingleChoiceModeListener(getListView(), adapter, BrowserWeiboMsgFragment.this, commentList.getItemList().get(
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if (position - listView.getHeaderViewsCount() < repostList.getSize() && position >= listView.getHeaderViewsCount()) {
				startActivity(BrowserWeiboMsgActivity.newIntent(GlobalContext.getInstance().getAccountBean(),
						repostList.getItemList().get(position - listView.getHeaderViewsCount()), GlobalContext.getInstance().getSpecialToken()));
			} else {
				loadOldRepostData();
			}
		}
	...
	private AbsListView.OnScrollListener listViewOnScrollListener = new AbsListView.OnScrollListener() {
		...
		public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
			...
			if (getListView().getLastVisiblePosition() > 7 && getListView().getFirstVisiblePosition() != getListView().getHeaderViewsCount()) {
