...
import org.zarroboogs.weibo.RepostWeiboMainActivity;
...
import org.zarroboogs.weibo.ui.common.CommonProgressDialogFragment;
import org.zarroboogs.weibo.ui.interfaces.AbstractAppActivity;
import org.zarroboogs.weibo.ui.loader.AbstractAsyncNetRequestTaskLoader;
...
public class BrowserWeiboMsgActivity extends AbstractAppActivity implements RemoveWeiboMsgDialog.IRemove {
	...
	private LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<MessageBean>> refreshCallback = new LoaderManager.LoaderCallbacks<AsyncTaskLoaderResult<MessageBean>>() {
		...
		@Override
		public void onLoadFinished(Loader<AsyncTaskLoaderResult<MessageBean>> loader, AsyncTaskLoaderResult<MessageBean> result) {
			...
			new Handler(Looper.getMainLooper()).post(new Runnable() {
				@Override
				public void run() {
					CommonProgressDialogFragment dialog = (CommonProgressDialogFragment) getSupportFragmentManager().findFragmentByTag(
					...
					if (exception != null) {
						CommonErrorDialogFragment userInfoActivityErrorDialog = CommonErrorDialogFragment.newInstance(exception.getError());
						getSupportFragmentManager().beginTransaction().add(userInfoActivityErrorDialog, CommonErrorDialogFragment.class.getName()).commit();
					}
				}
			});
