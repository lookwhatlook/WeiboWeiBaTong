...
import org.zarroboogs.weibo.ui.adapter.CommentListAdapter;
...
import org.zarroboogs.weibo.ui.loader.CommentsToMeDBLoader;
import org.zarroboogs.weibo.ui.loader.CommentsToMeMsgLoader;
import org.zarroboogs.weibo.ui.main.CommentsTimeLine;
...
public class CommentsToMeTimeLineFragment extends AbstractTimeLineFragment<CommentListBean> implements IRemoveItem {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		case ACTIVITY_DESTROY_AND_CREATE:
			...
			CommentListBean savedBean = (CommentListBean) savedInstanceState.getSerializable(Constances.BEAN);
			...
			if (position - 1 < getList().getSize() && position - 1 >= 0) {
				if (actionMode != null) {
					...
					actionMode = getActivity().startActionMode(
							new CommentSingleChoiceModeListener(getListView(), timeLineAdapter, CommentsToMeTimeLineFragment.this, getList().getItemList().get(
									position - 1)));
					return true;
				} else {
					getListView().setItemChecked(position, true);
	...
	protected Loader<AsyncTaskLoaderResult<CommentListBean>> onCreateMiddleMsgLoader(int id, Bundle args, String middleBeginId, String middleEndId,
	...
	private void clearUnreadComment(final UnreadBean data) {
		new MyAsyncTask<Void, Void, Void>() {
			...
			protected Void doInBackground(Void... params) {
				try {
					new ClearUnreadDao(GlobalContext.getInstance().getAccountBean().getAccess_token()).clearCommentUnread(data, GlobalContext.getInstance()
